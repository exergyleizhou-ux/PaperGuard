...
**Decision rule:** if your reference LM is OpenAI gpt-4o-family,
set the env var. If it's a self-hosted Llama-3.x (vLLM-style) or
a base / lightly-RLHF'd model, keep the default. If you're unsure,
run the controlled benchmark (`scripts/t7_controlled_benchmark.py`)
on your endpoint first and check whether `mean(AI ppl)` is above or
below `mean(human ppl)`.
...
**Partial empirical validation of the RLHF-suppression hypothesis
(2.4.2):** the two endpoints that DO return real per-token logprobs
on the same 10+10 corpus disagree on direction in a way that
matches their RLHF aggressiveness:

| Endpoint | Direction | LR+ | RLHF aggressiveness on LLM-markers |
|---|---|---|---|
| Groq `qwen/qwen3-32b` | textbook (AI ppl < human ppl) | 1.69 (weak) | moderate (Alibaba alignment) |
| OpenAI `gpt-4o` | **REVERSED** (AI ppl > human ppl) | 8.0 inverted (strong) | aggressive ("speak naturally" RLHF objective) |

Full validation would require a self-hosted Llama-3.x reference LM
with real logprobs (vLLM-style), which neither Groq nor the
current proxies expose; this is left as future work. Groq does not
expose logprobs on any Llama variant ("`logprobs` is not supported
with this model" on `llama-3.1-8b-instant`, `llama-3.3-70b-versatile`,
`meta-llama/llama-4-scout-17b-16e-instruct`), only on `qwen/qwen3-32b`,
as of 2026-05-23.
