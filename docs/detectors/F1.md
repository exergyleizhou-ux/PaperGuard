# F1 — Image Duplication (pHash)

> 对一组图像计算 perceptual hash，找出疑似重复或近似变体。
...
See [`detect()` signature](../../src/paperguard/detectors/f1_image_duplication.py) for
exact type expectations.

## Method

See implementation: [`src/paperguard/detectors/f1_image_duplication.py`](../../src/paperguard/detectors/f1_image_duplication.py)
...
Every finding from `F1` carries a non-exhaustive list of plausible
...
[`f1_image_duplication.py`](../../src/paperguard/detectors/f1_image_duplication.py).
...
Bik et al. (2016) Inappropriate image duplication studies; standard perceptual-hash forensics.

## Source

- Implementation: [`src/paperguard/detectors/f1_image_duplication.py`](../../src/paperguard/detectors/f1_image_duplication.py)
