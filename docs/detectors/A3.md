...
Cluster: `inter_column_relation` — findings from detectors in the same cluster
are treated as non-independent during the BH-FDR + severity escalation
in [`combiner.py`](../../src/paperguard/evidence/combiner.py).

## Inputs

This detector requires data shaped as: `raw_numeric_values_multi_column`.

See [`detect()` signature](../../src/paperguard/detectors/a3_arithmetic.py) for
exact type expectations.

## Method

See implementation: [`src/paperguard/detectors/a3_arithmetic.py`](../../src/paperguard/detectors/a3_arithmetic.py)
...
Every finding from `A3` carries a non-exhaustive list of plausible
...
[`a3_arithmetic.py`](../../src/paperguard/detectors/a3_arithmetic.py).
...
Mosimann et al. (1995) fabrication heuristics; independent measurements should accumulate noise in differences/ratios.

## Source

- Implementation: [`src/paperguard/detectors/a3_arithmetic.py`](../../src/paperguard/detectors/a3_arithmetic.py)
