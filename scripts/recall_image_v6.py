"""recall_image_v6 — PubMed-indexed both arms (uniform corpus source).

v5 had asymmetric arm attrition: 132/200 retracted pdf_ok but only
48/95 control pdf_ok. The asymmetry came from matched-control
papers having less uniform OA availability than retracted papers.

v6 originally aimed for `has_pmcid:true` (PMC full-text guarantee).
That filter intersects emptily with `is_retracted:true` on OpenAlex
— retracted papers' PMC IDs are sparsely populated relative to
their PMIDs (verified 2026-05-23: 0 results, vs 17 k for the same
query with `has_pmid:true`). v6 therefore uses `has_pmid:true`
instead: both arms restricted to PubMed-indexed (biomedical)
papers, OA fetch path still prefers Europe PMC via the existing
`paperguard.fetcher.oa_pdf`.

Expected effect: control-arm pdf_ok rate jumps from ~50 % (v5) to
closer to the retracted arm's rate (~65–70 %); per-arm n stays
closer to the target N; Wilson CIs on F4/F6 LR+ tighten.

Rest of methodology is byte-identical to v4/v5:
- Three detectors per paper: F1 (intra-paper pHash), F4
  (cross-paper corpus), F6 (per-channel histogram patch splice).
- Per-image F6 result aggregated as max-per-paper.
- Severity tiers at z=6/cluster=8 (the 2.1.9 calibrated defaults).

Outputs ``scripts/recall_image_v6_results.json``. Resumable via
``--resume``.
"""
        ...
        data = fetch(
            ...
            params={
                "filter": (
                    ...
                    "type:article,language:en,has_pmid:true,"
...
def get_matched_control(retracted: dict[str, Any]) -> dict[str, Any] | None:
    ...
    filt = (
        ...
        f"language:en,has_pmid:true,"
        f"primary_topic.subfield.id:{subfield_short},"
